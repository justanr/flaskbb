...
def flaskbb_gather_password_validators(app):
    """
    Hook for gathering :class:`~flaskbb.core.user.update.PasswordValidator`
    instances. This hook should return an iterable::

        class NotLongEnough(PasswordValidator):
            def __init__(self, min_length):
                self._min_length = min_length

            def validate_password(self, user, password_change):
                if len(password_change.new_password) < self._min_length:
...
def flaskbb_gather_email_validators(app):
    """
    Hook for gathering :class:`~flaskbb.core.user.update.EmailValidator`
    instances. This hook should return an iterable::

        class BlackListedEmailProviders(EmailValidator):
            def __init__(self, black_list):
                self._black_list = black_list

            def validate_email(self, user, email_change):
                provider = email_change.new_email.split('@')[1]
...
def flaskbb_gather_details_update_validators(app):
    """
    Hook for gathering :class:`~flaskbb.core.user.update.UserDetailValidator`
    instances. This hook should return an iterable::

        class DontAllowImageSignatures(UserDetailValidator):
            def __init__(self, renderer):
                self._renderer = renderer

            def validate_details(self, user, details_change):
                rendered = self._renderer.render(details_change.signature)
                if '<img' in rendered:
...
def flaskbb_details_updated(user, details_update):
    ...
    See also :class:`~flaskbb.core.user.update.UserDetailsUpdatePostProcessor`
...
def flaskbb_password_updated(user):
    ...
    See also :class:`~flaskbb.core.user.update.UserPasswordUpdatePostProcessor`::
...
def flaskbb_email_updated(user, email_update):
    ...
    See also :class:`~flaskbb.core.user.update.UserEmailUpdatePostProcessor`::
...
def flaskbb_settings_updated(user, settings_update):
    ...
    See also :class:`~flaskbb.core.user.update.UserSettingsUpdatePostProcessor`.
