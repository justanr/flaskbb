...
@attr.s(cmp=True, hash=True, repr=True, frozen=True, slots=True)
class NavigationText(NavigationItem):
    """
    Representation of plain text shown in a navigation bar::

        NavigationText(
            text="Some text",
            icon="fa fa-ellipsis-h"  # optional
        )
    """

    text = attr.ib()
    icon = attr.ib(default="")
    content_type = NavigationContentType.text
