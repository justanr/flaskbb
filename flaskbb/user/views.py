...
class UserSettings(MethodView):
    ...
    def post(self):
        if self.form.validate_on_submit():
            try:
                self.settings_update_handler.update_settings(
...
class ChangePassword(MethodView):
    ...
    def post(self):
        if self.form.validate_on_submit():
            try:
                self.password_update_handler.update_password(
                    current_user, self.form.as_change()
                )
            except StopValidation as e:
                self.form.populate_errors(e.reasons)
...
class ChangeEmail(MethodView):
    ...
    def post(self):
        if self.form.validate_on_submit():
            try:
                self.update_email_handler.update_email(
                    current_user, self.form.as_change()
                )
            except StopValidation as e:
                self.form.populate_errors(e.reasons)
...
class ChangeUserDetails(MethodView):
    ...
    def post(self):
        ...
        if self.form.validate_on_submit():
            try:
                self.details_update_handler.update_details(
                    current_user, self.form.as_change()
                )
            except StopValidation as e:
                self.form.populate_errors(e.reasons)
